            ...
            <v-card-text class="pa-0">
              <v-data-table
                ...
                <td>
                  ...
                  <td>{{ props.index+1 }}</td>
                  <td>{{ props.item.tax_class }}</td>
                  <td>{{ props.item.tax_rate }}</td>
                  <td>{{ props.item.based }}</td>
                  <td>{{ props.item.priority }}</td>
                  <td>
                    <v-btn @click="viewEdit(props.item.tax_rule_id)" depressed outline icon fab dark color="primary" small>
                      <v-icon>edit</v-icon>
                    </v-btn>
                    <v-btn @click="btnConfirm(props.item.tax_rule_id)"  depressed outline icon fab dark color="red" small>
...
<script>
  ...
  import {fetchList,updateData,deleteData} from '~/api/system/taxRule'
  ...
  export default {
    data () {
      return {
        routeName: 'TaxRuleForm',
        routeUrl: '/system/tax_rule',
        search: '',
        items: [],
        notifyStatus: '',
        message: '',
